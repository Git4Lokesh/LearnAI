...
import { isZodV3Schema, zodToJsonSchema, z } from '@llamaindex/core/zod';
 ...
 */ const createEventEmitterTool = (name, event, workflowContext, description)=>{
    ...
    const eventSchemaDescription = isZodV3Schema(event.schema) ? event.schema.description : null;
    const toolDescriptionWithSchema = (description ?? eventSchemaDescription ?? "Use this tool to send the event to the workflow.") + `\n\nPlease provide the event data in the following JSON schema: ${JSON.stringify(zodToJsonSchema(z.object({
